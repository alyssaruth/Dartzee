...
class TestTeamRenderer: AbstractTest()
{
    ...
    {
        renderer.getRendererComponent(row = rowIndex, isSelected = true).background shouldBe rawColour
        renderer.getRendererComponent(row = rowIndex, isSelected = false).background shouldBe rawColour.translucent()
    }

    private fun TableCellRenderer.getRendererComponent(isSelected: Boolean = false, hasFocus: Boolean = false, row: Int = 0, column: Int = 0) =
