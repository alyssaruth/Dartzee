...
class TestAchievementGolfOneHitWonder : AbstractAchievementTest<AchievementGolfOneHitWonder>()
{
    ...
    fun `Should pick the game with the highest count`()
    {
        ...
        val dartRounds1 = listOf(
            makeGolfRound(1, listOf(drtOuterOne(), drtDoubleOne()))
        )
        dartRounds1.flatten().forEach {
            insertDart(pt1, it, dtCreation = Timestamp(500))
        }

        val dartRounds2 = listOf(
            makeGolfRound(1, listOf(drtOuterOne(), drtDoubleOne())),
            makeGolfRound(2, listOf(drtDoubleTwo())),
        )
        dartRounds2.flatten().forEach {
            insertDart(pt2, it, dtCreation = Timestamp(1000))
        }

        runConversion()

        val a = AchievementEntity.retrieveAchievement(AchievementType.GOLF_ONE_HIT_WONDER, player.rowId)!!
