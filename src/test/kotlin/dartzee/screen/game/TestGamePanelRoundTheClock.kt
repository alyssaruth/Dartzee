...
class TestGamePanelRoundTheClock : AbstractTest() {
    ...
    fun `Should add on to the current streak if one exists`() {
        ...
        val dartsThrown =
            listOf(factoryClockHit(1), factoryClockHit(2), factoryClockHit(3), factoryClockHit(4))
        panel.addCompletedRound(dartsThrown)

        val achievement =
            AchievementEntity.retrieveAchievement(AchievementType.CLOCK_BEST_STREAK, playerId)!!
    ...
    fun `Should update the dartboard when a dart is thrown`() {
        ...
        panel.dartboard
            .segmentStatuses()!!
            .scoringSegments
            .shouldContainExactly(getAllNonMissSegments().filter { it.score == 2 })
    }
    ...
    fun `Should unlock the correct achievements for team play`() {
        ...
        val roundTwo =
            listOf(factoryClockHit(5), factoryClockHit(6), factoryClockHit(7), factoryClockHit(8))
        panel.addCompletedRound(roundTwo)
