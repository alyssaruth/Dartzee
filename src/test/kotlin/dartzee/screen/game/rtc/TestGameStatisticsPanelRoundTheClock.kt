...
class TestGameStatisticsPanelRoundTheClock :
    AbstractGameStatisticsPanelTest<ClockPlayerState, GameStatisticsPanelRoundTheClock>() {
    override fun factoryStatsPanel() =
        GameStatisticsPanelRoundTheClock(RoundTheClockConfig(ClockType.Standard, true).toJson())
    ...
    fun `Should separate out participants when calculating most fewest and avg darts`() {
        ...
        val stateTwo =
            makeClockPlayerState(player = player, completedRounds = listOf(roundOneOther))
    ...
    fun `Should take into account the gameParams when calculating best streak`() {
        ...
        val state = makeClockPlayerState(completedRounds = listOf(roundOne))
        val statsPanel =
            GameStatisticsPanelRoundTheClock(RoundTheClockConfig(ClockType.Doubles, true).toJson())
    ...
    fun `Should put a finished streak into the correct breakdown category`() {
        ...
        // 12
        state =
            state.copy(completedRounds = mutableListOf(missRound, missRound, missRound, hitRound))
        statsPanel.showStats(listOf(state))
        ...
        state =
            state.copy(
                completedRounds =
                    mutableListOf(missRound, missRound, missRound, missRound, missRound, hitRound)
            )
        statsPanel.showStats(listOf(state))
