...
class TestForeignDatabaseValidator: AbstractTest()
{
    ...
    fun `Should return false if remote database has higher version`()
    {
        usingInMemoryDatabase { remoteDatabase ->
            ...
            dialogFactory.errorsShown.shouldContainExactly("The other database contains data written by a higher Dartzee version. \n\nYou will need to update to the latest version of Dartzee before continuing.")
        }
    }
    ...
    fun `Should migrate remote database to latest version and return true on success`()
    {
        usingInMemoryDatabase(withSchema = true) { remoteDatabase ->
            ...
            val migrations = mapOf(
                    dbVersion to listOf { database: Database -> database.executeUpdate("CREATE TABLE Test(RowId VARCHAR(36))") }
            )
    ...
    private fun makeValidator(databaseMigrator: DatabaseMigrator = DatabaseMigrator(emptyMap())) = ForeignDatabaseValidator(databaseMigrator)
}