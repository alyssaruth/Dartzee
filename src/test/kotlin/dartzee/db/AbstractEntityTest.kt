...
abstract class AbstractEntityTest<E : AbstractEntity<E>> : AbstractTest() {
    ...
    fun `Should be bulk insertable`() {
        ...
        mainDatabase.executeQueryAggregate(
            "SELECT COUNT(1) FROM $tableName WHERE DtLastUpdate = ${getEndOfTimeSqlString()}"
        ) shouldBe 0
    ...
    fun `Column names should match declared fields`() {
        getExpectedClassFields().forEach { dao.javaClass.getMethod("get$it") shouldNotBe null }
    }
    ...
    fun `Delete where`() {
        ...
        if (factory().includeInSync()) {
            ...
            audits
                .map { it.entityId }
                .shouldContainExactlyInAnyOrder(entityOne.rowId, entityThree.rowId)
    ...
    fun `Insert and retrieve`() {
        ...
        val insertLog =
            flushAndGetLogRecords().find {
                it.loggingCode == CODE_SQL &&
    ...
    fun `Update and retrieve`() {
        ...
        val updateLog =
            flushAndGetLogRecords().find {
                it.loggingCode == CODE_SQL && it.message.contains("UPDATE ${dao.getTableName()}")
            }
    ...
    fun `Columns should not allow NULLs`() {
        ...
        getExpectedClassFields().forEach {
            ...
            log.errorObject?.message shouldContain
                "Column '${it.uppercase()}'  cannot accept a NULL value."
        }
    }
    ...
    fun `Should be able to insert and update in specified database instance`() {
        usingInMemoryDatabase { db ->
            val entity: AbstractEntity<E> =
                dao.javaClass.getDeclaredConstructor(Database::class.java).newInstance(db)
            entity.createTable()
    ...
    private fun <T> getValueForField(fieldType: Class<T>, initial: Boolean): Any {
        return when (fieldType) {
            ...
            Timestamp::class.java ->
                if (initial) Timestamp.valueOf("2019-04-01 21:29:32") else DateStatics.END_OF_TIME
            ...
            AchievementType::class.java ->
                if (initial) AchievementType.X01_BEST_FINISH else AchievementType.CLOCK_BEST_GAME
