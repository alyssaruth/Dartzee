...
class AmazonS3RemoteDatabaseStore(private val bucketName: String) : IRemoteDatabaseStore {
    ...
    override fun databaseExists(remoteName: String) =
        s3Client.doesObjectExist(bucketName, getCurrentDatabaseKey(remoteName))
    ...
    private fun verifyLastModifiedNotChanged(remoteName: String, lastModified: Date) {
        ...
        if (metadata.lastModified > lastModified) {
            throw ConcurrentModificationException(
                "Remote database $remoteName was last modified ${metadata.lastModified}, which is after $lastModified - aborting"
            )
        }
    }
