...
class Logger(private val destinations: List<ILogDestination>) {
    ...
    fun warn(code: LoggingCode, message: String, vararg keyValuePairs: Pair<String, Any?>) {
        warn(code, message, null, keyValuePairs = keyValuePairs)
    }
    ...
    fun warn(
        ...
        log(Severity.WARN, code, message, errorObject, mapOf(*keyValuePairs))
    }
    ...
    fun logSqlException(
        ...
        error(
            CODE_SQL_EXCEPTION,
            "Caught SQLException for statement: $sqlStatement",
            sqlException,
            KEY_SQL to sqlStatement,
            KEY_GENERIC_SQL to genericStatement,
