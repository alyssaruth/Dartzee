...
open class GameStatisticsPanelGolf : AbstractGameStatisticsPanel<GolfPlayerState>() {
    ...
    override fun getRankedRowsLowestWins() =
        listOf("Best Hole", "Avg. Hole", "Worst Hole", "Miss %", "Points Squandered")
    ...
    private fun getMissesRow() =
        prepareRow("Miss %") { playerName ->
    ...
    private fun getGambleRow(f: (rnd: List<Dart>) -> Int, desc: String) =
    ...
    private fun getScoreCountRow(score: Int) =
        getScoreRow("$score") { scores -> scores.count { it == score } }

    private fun getScoreRow(desc: String, f: (golfScores: List<Int>) -> Any?) =
        prepareRow(desc) { playerName ->
