...
import dartzee.achievements.getIdsStr
        ...
        protected val totalPlayers: Int) : PanelWithScorers<S>(), DartboardListener, ActionListener, MouseListener
{
    ...
    protected fun getActiveCount() = getParticipants().count { it.getTeam().isActive() }
    ...
    private fun loadScoresAndCurrentPlayer(gameId: String)
    {
        ...
        for (i in 0 until totalPlayers)
        {
            ...
            val sql = ("SELECT drt.RoundNumber, drt.Score, drt.Multiplier, drt.PosX, drt.PosY, drt.SegmentType, drt.StartingScore"
                    + " FROM Dart drt"
                    + " WHERE drt.ParticipantId IN ${individuals.getIdsStr { it.rowId }}"
                    + " AND drt.PlayerId IN ${individuals.getIdsStr { it.playerId }}"
    ...
    private fun isActive(playerNumber: Int) = getParticipant(playerNumber).getTeam().isActive()
