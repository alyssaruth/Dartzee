...
data class DartsAiModel(
    ...
    private fun getDistributionToUse(pt: Point) =
        if (standardDeviationDoubles != null && AI_DARTBOARD.isDouble(pt))
            NormalDistribution(mean.toDouble(), standardDeviationDoubles)
        else distribution
    ...
    private fun generateAngle(pt: Point): Double {
        ...
        // Otherwise, we have a Normal Distribution to use to generate an angle more likely to be
        // into the dartboard (rather than out of it)
    ...
    companion object {
        val DEFAULT_GOLF_SEGMENT_TYPES =
            mapOf(1 to SegmentType.DOUBLE, 2 to SegmentType.TREBLE, 3 to SegmentType.TREBLE)
