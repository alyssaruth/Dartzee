...
object DartsAiSimulator {
    fun runSimulation(model: DartsAiModel, dartboard: PresentationDartboard): SimulationWrapper {
        ...
        val hmPointToCount =
            scoringPoints.groupBy { dartboard.interpretPoint(it) }.mapValues { it.value.size }
        ...
        val trebles =
            scoringSegments.count { it.type == SegmentType.TREBLE && it.score == model.scoringDart }
    ...
    private fun throwScoringDarts(model: DartsAiModel) =
        (0 until SCORING_DARTS_TO_THROW).map { model.throwScoringDart() }
