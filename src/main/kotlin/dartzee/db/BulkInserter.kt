...
object BulkInserter {
    ...
    fun insert(
        ...
        val threads =
            entitiesBatched.map {
                getInsertThreadForBatch(it, tableName, rowsPerStatement, database)
            }
    ...
    private fun getInsertThreadForBatch(
        ...
        return Thread {
            batch.chunked(rowsPerInsert).forEach { entities ->
                val genericInsert =
                    "INSERT INTO $entityName VALUES ${entities.joinToString{it.getInsertBlockForStatement()}}"
                ...
                try {
                    conn.prepareStatement(insertQuery).use { ps ->
                        entities.forEachIndexed { index, entity ->
                            ...
                            insertQuery =
                                entity.writeValuesToInsertStatement(insertQuery, ps, index)
                        }
