...
class DartzeeRuleEntity(database: Database = mainDatabase) :
    AbstractEntity<DartzeeRuleEntity>(database) {
    ...
    fun retrieveForTemplate(templateId: String) =
        retrieveEntities(getTemplateWhere(templateId)).sortedBy { it.ordinal }

    fun deleteForTemplate(templateId: String) = deleteWhere(getTemplateWhere(templateId))

    fun retrieveForGame(gameId: String) =
        retrieveEntities(getGameWhere(gameId)).sortedBy { it.ordinal }

    fun deleteForGame(gameId: String) = deleteWhere(getGameWhere(gameId))

    private fun getTemplateWhere(templateId: String) =
        "EntityName = '${EntityName.DartzeeTemplate}' AND EntityId = '$templateId'"

    private fun getGameWhere(gameId: String) =
        "EntityName = '${EntityName.Game}' AND EntityId = '$gameId'"
}
