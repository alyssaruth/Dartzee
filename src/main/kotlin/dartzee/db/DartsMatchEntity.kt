...
/**
 * Simple entity to join multiple 'games' together into a 'match'. Table has to be called
 * 'DartsMatch' because 'Match' is a derby keyword!
 */
class DartsMatchEntity(database: Database = mainDatabase) :
    AbstractEntity<DartsMatchEntity>(database) {
    /** DB Fields */
    var localId = -1L
    var games = -1
    ...
    fun getMatchDesc() =
        "Match #$localId (${getMatchTypeDesc()} - ${gameType.getDescription(gameParams)})"
    ...
    fun getScoreForFinishingPosition(position: Int) =
        when (mode) {
            ...
            MatchMode.POINTS ->
                if (position == -1) 0 else getHmPositionToPoints().getValue(position)
        }
    ...
    companion object {
        ...
        fun factoryPoints(games: Int, pointsJson: String) =
            factoryAndSave(games, MatchMode.POINTS, pointsJson)
